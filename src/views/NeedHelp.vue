<template>
  <div>
    <div class="content gap-2 grid grid-cols-2 px-8 xl:grid-cols-3">
      <Card :url="{ name: 'NeedHelpShow', params: { city: $store.getters.getCity, postId: 1 } }">
      ...
      <Card :url="{ name: 'NeedHelpShow', params: { city: $store.getters.getCity, postId: 2 } }">
        <template slot="logo"></template>
        ...
        <template slot="ingress">
          ...
          <br />Det allra bästa sättet för er att hjälpa oss är att lyssna på våra
          myndigheters råd för att undvika att bli sjuka och belasta våra
          resurser
        </template>
      </Card>
      <Card :url="{ name: 'NeedHelpShow', params: { city: $store.getters.getCity, postId: 3 } }">
        <template slot="logo"></template>
      ...
      <Card :url="{ name: 'NeedHelpShow', params: { city: $store.getters.getCity, postId: 4 } }">
        <template slot="logo"></template>
